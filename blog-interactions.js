(function(){
  ...
  var TEXT={
    calculations:{en:'Associated PDF: detailed calculations',es:'PDF asociado: cálculos detallados',de:'Zugehöriges PDF: detaillierte Rechnungen',zh:'相关 PDF：详细计算'},
  ...
  function pdfButton(id){var p=POSTS[id];return '<a class="action-btn pdf-tech" data-pdf-post="'+id+'" href="'+escapeHtml(tx(p.pdf))+'" target="_blank" rel="noopener">'+
    '<svg class="pdf-sparks" viewBox="0 0 100 30" preserveAspectRatio="none" aria-hidden="true"><rect class="pdf-ring" x="1.5" y="1.5" width="97" height="27" rx="13.5" pathLength="100"/><rect class="pdf-spark spark-3" x="1.5" y="1.5" width="97" height="27" rx="13.5" pathLength="100"/><rect class="pdf-spark spark-2" x="1.5" y="1.5" width="97" height="27" rx="13.5" pathLength="100"/><rect class="pdf-spark" x="1.5" y="1.5" width="97" height="27" rx="13.5" pathLength="100"/></svg><span aria-hidden="true">↓</span>'+ml(TEXT.calculations)+'</a>';}
  function shareBlock(id){return '<div class="share-wrap"><button class="action-btn share-toggle" type="button" data-share-toggle="'+id+'" aria-expanded="false">'+icon('share')+ml(TEXT.share)+'</button><div class="share-menu" data-share-menu="'+id+'">'+
  ...
  function installIndividual(){var id=postIdFromPath();if(!id)return;var meta=document.querySelector('.papermeta');if(meta&&!document.querySelector('.top-pdf-action'))meta.insertAdjacentHTML('afterend','<div class="top-pdf-action">'+pdfButton(id)+'</div>');var aidisc=document.querySelector('.aidisc');if(!aidisc)return;aidisc.insertAdjacentHTML('beforebegin',interactionPanel(id));}
  function installPreviews(){document.querySelectorAll('#bf article.post,#bt article.post').forEach(function(card){var link=card.querySelector('.plink');var id=link&&postIdFromPath(link.getAttribute('href'));if(!id||card.querySelector('.preview-actions'))return;card.insertAdjacentHTML('beforeend','<div class="preview-actions">'+pdfButton(id)+reactionButtons(id,true)+shareBlock(id)+'</div>');});}
  ...
  function init(){installIndividual();installPreviews();installGate();initClient();bindReactions();bindComments();bindPdfLanguage();bindShare();observeLanguage();load(Object.keys(POSTS).filter(function(id){return document.querySelector('[data-reactions="'+id+'"]');}));}
