(function(){
  ...
  function installMaterials(){document.querySelectorAll('.docitem').forEach(function(card){var head=card.querySelector('.dochead'),url=head&&head.dataset.url;if(!url||card.querySelector('.material-actions'))return;var id='mat-'+stableHash(url),anchor='material-'+id.slice(4),title=((card.querySelector('.doctitle .lang-en')||card.querySelector('.doctitle')||{}).textContent||document.title).trim(),path=location.pathname.replace(/\.html$/,'');card.id=anchor;card.dataset.materialId=id;head.insertAdjacentHTML('afterend','<div class="material-actions">'+reactionButtons(id,true)+shareBlock(id,path+'#'+anchor,title)+'</div>');});}
